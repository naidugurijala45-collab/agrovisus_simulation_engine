...
def diagnose_v3():
    print("--- 🩺 Diagnosing V3 Agent Failure ---")
    config = load_config()
    ...
    while not (term or trunc):
        ...
        if day > 20 and total_reward < -50:
            ...
            # Fast forward? No, just stop printing detailed logs but finish?
            # Actually, usually failure happens early. So 20 days is enough.
            if day > 30: 
                break 

    print("-" * 70)
    print(f"Episode Finished. Total Reward: {total_reward:.2f}")
    if term: print("Result: TERMINATED (Death or Completion)")
    if trunc: print("Result: TRUNCATED")
