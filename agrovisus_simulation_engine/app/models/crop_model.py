...
class CropModel:
    ...
    def update_daily(
        ...
        # --- Root Growth (New) ---
        # Roots grow during Emergence and Vegetative stages, stop at Flowering
        if self.current_stage in ["Emergence", "Vegetative"]:
             # Stress stunts root growth too
        ...
        # --- Biomass Partitioning (New) ---
        # Before flowering: all to Veg. After: all to Rep (Simplified).
        # We assume stages like "Flowering", "GrainFilling", "Maturation" are reproductive.
        reproductive_stages = ["Flowering", "GrainFilling", "Maturation"]
        
        if self.current_stage in reproductive_stages:
