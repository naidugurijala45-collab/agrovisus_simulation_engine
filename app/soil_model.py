...
if __name__ == '__main__':
    ...
    sandy_model = SoilModel(soil_type_name="sandy_loam", 
                            soil_depth_mm=500, 
    ...
    for i, day_input in enumerate(daily_inputs):
        print(f"  Day {i+1} - Input P={day_input['P']}, I={day_input['I']}, ET0={day_input['ET0']}, Kc={day_input['Kc']}")
        fluxes = sandy_model.update_daily(precipitation_mm=day_input["P"],
                                          irrigation_mm=day_input["I"], 
                                          et0_mm=day_input["ET0"], 
                                          crop_coefficient_kc=day_input["Kc"])
        status = sandy_model.get_soil_moisture_status()
        print(f"    -> Fluxes: DP={fluxes['deep_percolation_mm']:.1f}mm, Runoff={fluxes['runoff_mm']:.1f}mm, ETa={fluxes['actual_eta_mm']:.1f}mm")
        print(f"    -> Status: Water={status['current_water_mm']:.1f}mm, Frac_AWC={status['fraction_awc']:.2f}, Cat='{status['status_category']}'")