...
class NutrientModel:
    def __init__(self,
                 ...
                 temp_base: float, temp_opt: float, temp_max: float):
    ...
    def _simulate_transformations(self, temp_c: float, fraction_awc: float):
        ...
        logger.debug(f"N-Transform Factors: Temp={temp_factor:.2f}, Moist={moisture_factor:.2f}, Env={env_factor:.2f}")
        logger.debug(f"N-Transform Rates: Hydr.={hydrolysis_rate:.2f}, Nitr.={nitrification_rate:.2f}")
    ...
    def _simulate_leaching(self, deep_percolation_mm: float, soil_status: Dict[str, Any]) -> float:
        ...
        n_concentration = self.nitrate_N_kg_ha / current_water_mm if current_water_mm > 0 else 0
        n_leached_kg_ha = min(self.nitrate_N_kg_ha, max(0, n_concentration * deep_percolation_mm))
    ...
    def update_daily(self,
                     ...
                     avg_temp_c: float,
                     soil_status: Dict[str, Any]) -> float:
