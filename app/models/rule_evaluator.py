...
class RuleEvaluator:
    ...
    def _load_rules(self, file_path: str) -> List[Dict[str, Any]]:
        try:
            ...
            if "rules" not in rules_data or not isinstance(rules_data["rules"], list):
                logger.error("Rule file must have a top-level 'rules' key containing a list.")
                return []
    ...
    def _check_condition(self, condition: Dict[str, Any], input_data: Dict[str, Any]) -> bool:
        try:
            ...
            logger.debug(f"Data for '{e.args[0]}' (path: {path_parts}) not found or invalid in input_data. Condition fails.")
            return False
...
if __name__ == '__main__':
        ...
        "rules": [
            {
                ...
                "conditions": [
                    { "path": "crop.stage", "operator": "in", "threshold": ["V6", "V8", "V10", "VT"] },
                    { "path": "weather.leaf_wetness_hours", "operator": "greater_than", "threshold": 6 },
                    { "path": "weather.humidity_percent", "operator": "greater_than", "threshold": 80 }
                ],
                "result": {
            ...
            },
            {
                ...
                "conditions": [
                    { "path": "soil.fraction_awc", "operator": "less_than", "threshold": 0.3 }
                ],
                "result": {
    ...
    try:
        if os.path.exists(RULE_FILE_TEST): # This line now works because 'os' is imported
