...
class DiseaseModel:
    def __init__(self, config: Dict[str, Any]):
        ...
        self.max_severity_rate = config.get('max_severity_rate', 0.05) # Max daily % increase in severity
        ...
        self.min_precipitation_for_wetness_mm = config.get('min_precipitation_for_wetness_mm', 1.0)
        ...
        if self.initial_inoculum > 0:
            ...
            logger.info(f"DiseaseModel initialized with initial inoculum: {self.disease_severity:.4f}")
        else:
    ...
    def _calculate_temperature_factor(self, avg_temp_c: float) -> float:
        ...
        if avg_temp_c < self.temp_min or avg_temp_c > self.temp_max:
            ...
            return 1.0 - ((avg_temp_c - self.temp_optimum) / (self.temp_max - self.temp_optimum))
    ...
    def update_daily(self, 
                     ...
                     crop_non_disease_stress_factor: float) -> None:
        """
        ...
        if environmental_factor <= 0:
            logger.debug(f"DiseaseModel Day Update: No environmental suitability (Env Factor={environmental_factor:.2f}). Severity remains {self.disease_severity:.4f}")
        ...
        stress_susceptibility_multiplier = 1.0 + (1.0 - crop_non_disease_stress_factor) * 0.5 
        
        total_susceptibility_factor = stage_susceptibility * stress_susceptibility_multiplier
        total_susceptibility_factor = min(total_susceptibility_factor, 2.0) # Cap the total multiplier for realism
        ...
        daily_potential_increase = (self.max_severity_rate * 
        ...
        logger.debug(f"DiseaseModel Day Update: Temp Factor={temp_factor:.2f}, Wetness Factor={wetness_factor:.2f}, "
