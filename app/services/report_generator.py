...
class ReportGenerator:
    ...
    def __init__(self, templates_dir="app/templates", plots_subdir="plots"):
        ...
        self.plots_subdir = plots_subdir  # Where plots will be saved relative to report_output_dir
        logging.info(f"ReportGenerator initialized with template from {templates_dir}")

    def generate_scenario_report(self, simulation_csv_filepath, config_filepath, report_output_dir):
        """
        ...
        if df.empty:
            logging.error(f"Cannot generate report: No data loaded from {simulation_csv_filepath}")
            return None
        ...
        try:
            ...
            logging.warning(f"Config file not found: {config_filepath}. Report will miss config details.")
        ...
        # Crop Growth
        crop_cols = ['total_biomass_kg_ha', 'accumulated_gdd']
        ...
        if available_crop_cols:
            plot_paths['crop_growth_plot'] = plot_utils.plot_multiple_time_series(
                df, "Crop Growth & Development", "Value",
                y_columns=available_crop_cols,
                filename="crop_growth.png", output_dir=plots_output_path
            )
        
        # Water Dynamics
        water_cols = ['soil_fraction_awc', 'precipitation_mm', 'irrigation_mm', 'deep_percolation_mm']
        ...
        if available_water_cols:
            plot_paths['water_dynamics_plot'] = plot_utils.plot_multiple_time_series(
                df, "Soil Moisture & Water Dynamics", "mm / Fraction",
                y_columns=available_water_cols,
                filename="water_dynamics.png", output_dir=plots_output_path
            )
        
        # Nutrient Dynamics
        nitrogen_cols = ['n_nitrate_kg_ha', 'n_uptake_kg_ha', 'n_available_kg_ha']
        ...
        if available_nitrogen_cols:
            plot_paths['nitrogen_dynamics_plot'] = plot_utils.plot_multiple_time_series(
                df, "Nitrogen Pools & Uptake", "N (kg/ha)",
                y_columns=available_nitrogen_cols,
                filename="nitrogen_dynamics.png", output_dir=plots_output_path
            )
        
        # Stress Factors
        stress_cols = ['water_stress_factor', 'nitrogen_stress_factor', 'disease_stress_factor']
        ...
        if available_stress_cols:
            plot_paths['stress_factors_plot'] = plot_utils.plot_multiple_time_series(
                df, "Crop Stress Factors", "Stress Factor (0-1)",
                y_columns=available_stress_cols,
                filename="stress_factors.png", output_dir=plots_output_path
            )
        
        # Disease Progression
        if 'disease_severity' in df.columns:
            plot_paths['disease_progression_plot'] = plot_utils.plot_time_series(
                data_manager.get_daily_data('disease_severity'), "Disease Progression", "Severity (%)",
                filename="disease_progression.png", output_dir=plots_output_path
            )
        
        # Adjust plot paths to be relative for HTML embedding
        for key, path in plot_paths.items():
            if path:
                plot_paths[key] = os.path.join(self.plots_subdir, os.path.basename(path))

        # 4. Extract KPIs (Summary Charts data)
        final_biomass = data_manager.get_final_value('total_biomass_kg_ha') if not df.empty else 0.0
        harvest_index = config_params.get('crop_model_config', {}).get('harvest_index', 0.5)
        
        kpis = {
            'final_yield_kg_ha': final_biomass * harvest_index if final_biomass else 0.0,
            'total_irrigation_mm': data_manager.get_summary_stats('irrigation_mm').get('sum', 0.0),
            'total_n_leached_kg_ha': 0.0,  # Not available in current CSV
            'peak_disease_severity_percent': data_manager.get_summary_stats('disease_severity').get('max', 0.0),
            'triggered_rules': []
        }
        
        # Process triggered rules
        if 'triggered_rules' in df.columns:
            triggered_rules = df['triggered_rules'].apply(
                lambda x: [r.strip() for r in x.split(',') if r.strip()] if isinstance(x, str) else []
            ).explode().dropna().unique().tolist()
            # Clean up the triggered rules to remove empty strings or 'None' if any
            kpis['triggered_rules'] = [rule for rule in triggered_rules if rule and rule.lower() != 'none']

        # 5. Prepare data for HTML rendering
        context = {
            "scenario_name": os.path.basename(config_filepath).replace('.json', '') if config_filepath else "Unnamed Scenario",
            "simulation_id": os.path.basename(simulation_csv_filepath).replace('.csv', ''),
            ...
            "daily_data_html_table": df.head(5).to_html(classes='table table-striped', escape=False) if not df.empty else "",  # Display first 5 rows of raw data
        }

        # 6. Render and save the HTML report
        report_html_content = self.template.render(context)
        report_filename = os.path.join(report_output_dir, f"report_{context['simulation_id']}.html")
    ...
    def generate_comparison_report(self, scenario_data_paths, comparison_output_dir):
        ...
        for name, paths in scenario_data_paths.items():
            ...
            df = data_manager.df
            if df.empty:
                logging.warning(f"Skipping scenario '{name}': No data loaded from {paths['csv_filepath']}")
                continue
            all_dfs[name] = df
            ...
            try:
                ...
                logging.warning(f"Could not load config for scenario '{name}': {e}. KPIs might be incomplete.")

            # Extract KPIs (same logic as in generate_scenario_report)
            kpis = {
                'final_yield_kg_ha': data_manager.get_final_value('total_biomass_kg_ha') * config_params.get('crop_harvest_index', 1.0) if not df.empty else 0.0,
                'total_irrigation_mm': data_manager.get_summary_stats('daily_irrigation_mm').get('sum', 0.0),
                'total_n_leached_kg_ha': data_manager.get_summary_stats('nitrogen_daily_leaching_kg_ha').get('sum', 0.0),
                'peak_disease_severity_percent': data_manager.get_summary_stats('disease_severity_percent').get('max', 0.0),
                # Add any other KPIs you want to compare
            }
            all_kpis[name] = kpis

        if not all_dfs:
            logging.error("No valid scenarios to compare. Aborting comparison report generation.")
            return None
        ...
        plot_paths['yield_comparison_plot'] = plot_utils.plot_comparative_bar_chart(
            all_kpis, 'final_yield_kg_ha', "Final Grain Yield Comparison", "Yield (kg/ha)",
            filename="comp_yield.png", output_dir=plots_output_path
        )
        plot_paths['irrigation_comparison_plot'] = plot_utils.plot_comparative_bar_chart(
            all_kpis, 'total_irrigation_mm', "Total Irrigation Comparison", "Irrigation (mm)",
            filename="comp_irrigation.png", output_dir=plots_output_path
        )
        ...
        plot_paths['comparative_awc_plot'] = plot_utils.plot_comparative_time_series(
            all_dfs, 'fraction_awc', "Soil Moisture (Fraction AWC) Comparison", "Fraction AWC (0-1)",
            filename="comp_awc.png", output_dir=plots_output_path
        )
        plot_paths['comparative_biomass_plot'] = plot_utils.plot_comparative_time_series(
            all_dfs, 'total_biomass_kg_ha', "Total Biomass Accumulation Comparison", "Biomass (kg/ha)",
            filename="comp_biomass.png", output_dir=plots_output_path
        )
        plot_paths['comparative_stress_plot'] = plot_utils.plot_comparative_time_series(
            all_dfs, 'overall_stress_factor', "Overall Crop Stress Comparison", "Stress Factor (0-1)",
            filename="comp_stress.png", output_dir=plots_output_path
        )
        plot_paths['comparative_disease_plot'] = plot_utils.plot_comparative_time_series(
            all_dfs, 'disease_severity_percent', "Disease Severity Progression Comparison", "Severity (%)",
            filename="comp_disease.png", output_dir=plots_output_path
        )
        # Adjust plot paths to be relative for HTML embedding
        for key, path in plot_paths.items():
            if path:
