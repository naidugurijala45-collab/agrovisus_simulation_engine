...
def save_plot(fig, filename, output_dir="outputs/plots"):
    ...
    fig.savefig(filepath, bbox_inches='tight', dpi=300) # bbox_inches='tight' prevents labels/titles from being cut off
    plt.close(fig) # Close the figure to free memory
    logging.info(f"Plot saved to: {filepath}")
    return filepath # Return the path for HTML embedding


def plot_time_series(df_series, title, ylabel, filename="time_series.png", output_dir="outputs/plots"):
    """
...
def plot_multiple_time_series(df_data, title, ylabel, y_columns=None, filename="multiple_time_series.png", output_dir="outputs/plots"):
    """
    ...
    for i, col in enumerate(plot_cols):
        df_data[col].plot(ax=ax, label=col.replace('_', ' ').title(), color=colors[i], linewidth=2)
...
def plot_bar_chart(data_dict, title, ylabel, filename="bar_chart.png", output_dir="outputs/plots", color=None):
    """
...
def plot_comparative_time_series(df_dict, y_column, title, ylabel, filename="comparative_time_series.png", output_dir="outputs/plots"):
    """
    ...
    colors = sns.color_palette("tab10", n_colors=len(df_dict)) # Use a distinct color for each scenario
    ...
    for i, (scenario_name, df) in enumerate(df_dict.items()):
        if y_column in df.columns:
            ...
            logging.warning(f"Column '{y_column}' not found in scenario '{scenario_name}'.")
...
def plot_comparative_bar_chart(kpis_dict, kpi_name, title, ylabel, filename="comparative_bar_chart.png", output_dir="outputs/plots"):
    """
    ...
    for scenario_name, kpis in kpis_dict.items():
        if kpi_name in kpis:
            ...
            logging.warning(f"KPI '{kpi_name}' not found for scenario '{scenario_name}'.")

    if not scenario_labels:
        return None

    fig, ax = plt.subplots(figsize=(10, 6))
    sns.barplot(x=scenario_labels, y=kpi_values, ax=ax, palette="viridis")
