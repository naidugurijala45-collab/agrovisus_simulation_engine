...
def calculate_leaf_wetness_duration(hourly_weather_data: List[Dict[str, Any]],
                                    rh_threshold: float = 90.0,
                                    rain_threshold: float = 0.1) -> float:
    wet_hours = 0
    ...
    for hour_data in hourly_weather_data:
        try:
            ...
            print(f"Warning: Skipping invalid data point in LWD calculation: {hour_data}. Error: {e}")
            continue
...
if __name__ == '__main__':
    ...
    print(f"Example 3: Calculated LWD with empty data: {duration_empty} hours (Expected: 0.0)")
    ...
    print(f"Example 4: Calculated LWD with invalid data: {duration_invalid} hours (Expected: 0.0 with warnings)")
