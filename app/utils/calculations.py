...
def et0_hargreaves(t_avg: float, t_min: float, t_max: float, lat: float, day_of_year: date) -> float:
    """
...
def et0_penman_monteith(t_min: float, t_max: float, t_avg: float,
                        rh_avg: float, rs_mj_m2: float, u2_m_s: float,
                        lat: float, elevation_m: float, day_of_year: date) -> float:
    """
    ...
    try:
        ...
        if any(v is None or pd.isna(v) for v in [t_min, t_max, t_avg, rh_avg, rs_mj_m2, u2_m_s]):
            logger.warning("Missing one or more weather variables for Penman-Monteith. Falling back to Hargreaves.")
        ...
        et0_series = et.pm(tmean, wind, rs=rs, elevation=elevation_m, lat=lat, tmax=tmax, tmin=tmin, rh=rh)
        et0_value = et0_series.iloc[0]
        
        if pd.isna(et0_value) or et0_value < 0:
            logger.warning(f"Penman-Monteith calculation resulted in invalid value ({et0_value}). Falling back to Hargreaves.")
            return et0_hargreaves(t_avg, t_min, t_max, lat, day_of_year)
